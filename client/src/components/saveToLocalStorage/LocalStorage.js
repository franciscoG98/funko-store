...
export const loadState = () => {
    try {
        const serializedData = localStorage.getItem('guest')
...
export const saveState = (prod) => {
    try {
        const arr = [];
        let itemGuest = prod.Order.carrito.config.data;
        let serializedData = JSON.stringify(itemGuest)
        arr.push(serializedData)
        localStorage.setItem('guest', arr)
    } catch (error) {
